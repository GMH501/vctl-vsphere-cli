...
@click.option('--password', '-pwd', '-p',
              ...
              required=True)
@click.pass_context
def envs(ctx, username, password):
    name = ctx.name
    context = ctx.context
    try:
...
@guest.command()
@click.option('--from-guest',
              help='Initiate the file transfer from guest to client.',
              is_flag=True)
@click.option('--from-client',
              help='Initiate the file transfer from client to guest.',
              is_flag=True)
@click.option('--username', '-user', '-u',
              help='The desiderd state for the virtual machine.',
              required=True)
@click.option('--password', '-pwd', '-p',
              ...
              required=True)
@click.pass_context
def transfer(ctx, from_client, from_guest, username, password):
    name = ctx.name
    context = ctx.context
    try:
        context = load_context(context=context)
